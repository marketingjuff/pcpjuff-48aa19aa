...
export function FaltaPorPedidoTab() {
    ...
    const { data: cops = [] } = useQuery({
        queryKey: ["cops"],
        queryFn: async () => {
            const { data, error } = await supabase.from("cops").select("*");
            if (error)
                throw error;
            return (data ?? []);
        },
    ...
    const [popupPeca, setPopupPeca] = useState(null);
    const linhas = useMemo(() => {
        ...
        for (const p of pedidos) {
            ...
            if (busca) {
                ...
                const ped = String(p.pedido_olist ?? "").toLowerCase();
                if (!orc.includes(busca.toLowerCase()) && !ped.includes(busca.toLowerCase()))
                    continue;
            }
            const mapa = new Map();
            for (const it of itens) {
                ...
                let g = mapa.get(k);
                if (!g) {
                    g = { modelo: it.ps.modelo, cor: it.ps.cor, porTamanho: new Map(), faltaTotal: 0 };
                    mapa.set(k, g);
                }
            ...
            const grupos = Array.from(mapa.values()).filter((g) => {
                if (modeloFiltro !== "todos" && g.modelo !== modeloFiltro)
                    return false;
                if (corFiltro !== "todas" && g.cor !== corFiltro)
                    return false;
                return true;
            });
            if (grupos.length === 0)
    ...
    const tamanhosColunas = useMemo(() => {
        ...
        for (const l of linhas)
            for (const g of l.grupos)
                for (const t of g.porTamanho.keys())
                    set.add(t);
    ...
    const baixar = useMutation({
        mutationFn: async ({ pedido, observacao, baixas, }) => {
            const arr = (pedido.pecas_solicitadas ?? []).slice();
            const { data: ses } = await supabase.auth.getUser();
            const novoLog = [...(pedido.pecas_completadas_log ?? [])];
            for (const b of baixas) {
                if (b.qtd <= 0)
                    continue;
                const linha = { ...arr[b.idx] };
            ...
            const { error } = await supabase
                .from("pedidos")
                .update({ pecas_solicitadas: arr, pecas_completadas_log: novoLog })
                .eq("id", pedido.id);
            if (error)
                throw error;
        },
        ...
        onError: (e) => toast.error(e.message ?? "Erro na baixa."),
    });
    const itensDialog = useMemo(() => {
        if (!baixa)
            return [];
    ...
    return (<div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2 justify-between">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Input placeholder="Buscar orçamento/pedido Olist..." value={busca} onChange={(e) => setBusca(e.target.value)} className="h-9 w-[220px]"/>
          ...
          {(modeloFiltro !== "todos" || corFiltro !== "todas" || busca) && (<Button variant="ghost" size="sm" className="h-8 px-2 text-xs" onClick={() => { setBusca(""); setModeloFiltro("todos"); setCorFiltro("todas"); }}>
      ...
      {linhas.length === 0 ? (<Card><CardContent className="p-6 text-sm text-muted-foreground text-center">Nenhum pedido com peças faltantes.</CardContent></Card>) : (<div className="rounded-md border overflow-auto max-h-[70vh] tbl-congelada">
          <table className="text-[12.5px] leading-[1.2]" style={{ borderCollapse: "collapse", tableLayout: "fixed", minWidth: "100%" }}>
            <colgroup>
              <col style={{ width: 210 }}/>
              <col style={{ width: 100 }}/>
              <col style={{ width: 70 }}/>
              {tamanhosColunas.map((t) => (<col key={t} style={{ width: 48 }}/>))}
              <col style={{ width: 44 }}/>
              <col style={{ width: 52 }}/>
            </colgroup>
            <thead className="bg-muted/40 text-xs">
              <tr>
                ...
                {tamanhosColunas.map((t) => (<th key={t} className="px-1 py-2 text-center whitespace-nowrap">{t}</th>))}
                ...
                return (<Fragment key={grp.key}>
                    <tr key={`sub:${grp.key}`} className="border-t bg-muted/60 font-semibold leading-tight">
                      <td className="px-1 py-1.5 text-left" colSpan={3}>
                        <button onClick={() => toggle(grp.key)} className="inline-flex items-center gap-1 text-foreground hover:text-primary text-xs">
                      ...
                      {tamanhosColunas.map((t) => (<td key={t} className="px-1 py-1.5 text-center tabular-nums text-amber-700 text-xs">
                    ...
                    {isExp && grp.rows.map((r) => {
                        const hex = corHex(r.grupo.cor);
                        const fg = corTextoSobre(hex);
                        return (<tr key={`${r.pedido.id}|${r.grupo.modelo}|${r.grupo.cor}`} className="border-t hover:bg-accent/40 cursor-pointer leading-tight" onClick={() => setHistorico(r.pedido)}>
                          {r.primeira ? (<td className="px-1 py-1 align-middle break-words text-xs" rowSpan={r.rowSpan}>
                              <div>{r.pedido.orcamento ?? "—"}</div>
                              {r.pedido.pedido_olist && (<div className="text-[10px] text-muted-foreground">Olist {r.pedido.pedido_olist}</div>)}
                            </td>) : null}
                          ...
                          {tamanhosColunas.map((t) => {
                                ...
                                return (<td key={t} className="px-1 py-1 text-center tabular-nums">
                                {info ? (<button type="button" className="text-amber-700 font-semibold hover:underline text-xs" onClick={(e) => { e.stopPropagation(); setPopupPeca({ modelo: r.grupo.modelo, cor: r.grupo.cor, tamanho: t }); }} title="Ver romaneios e pedidos com esta peça">
                                    -{info.falta}
      ...
      {baixa && (<BaixaCopDialog open={!!baixa} onOpenChange={(o) => !o && setBaixa(null)} modelo={baixa.grupo.modelo} cor={baixa.grupo.cor} orcamento={baixa.pedido.orcamento ?? null} itens={itensDialog} onConfirm={async (observacao, baixas) => {
      ...
      <Dialog open={!!historico} onOpenChange={(o) => !o && setHistorico(null)}>
        <DialogContent className="max-w-[720px]">
          ...
          {historico && (historico.pecas_completadas_log?.length ?? 0) === 0 ? (<div className="text-sm text-muted-foreground">Nenhuma baixa registrada ainda.</div>) : historico ? (<div className="rounded-md border overflow-x-auto max-h-[60vh] overflow-y-auto">
              <table className="w-full text-[12.5px] leading-[1.2]">
                ...
                <tbody>
                  {[...(historico.pecas_completadas_log ?? [])].sort((a, b) => b.em.localeCompare(a.em)).map((log, i) => (<tr key={i} className="border-t">
      ...
      {popupPeca && (<FaltaPecaPopup open={!!popupPeca} onOpenChange={(o) => !o && setPopupPeca(null)} modelo={popupPeca.modelo} cor={popupPeca.cor} tamanho={popupPeca.tamanho} pedidos={pedidos} cops={cops} oficinas={oficinas}/>)}
    </div>);
}
